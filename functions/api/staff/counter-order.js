...
export const onRequestPost = async ({ request, env }) => {
  ...
  // Remember them so caller ID can name them next time they ring. Shared with
  // the website's own orders so both build ONE address book; gated on
  // pos.customerLookup inside rememberContact.
  await rememberContact({ name, phone: rawPhone, address, source: 'counter' }, env);
  await logAudit(env, {
